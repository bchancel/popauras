...
function Panel:ApplyCurrent(deferRefresh)
  if self.suppressUpdates then
    return
  end
  ...
  if trigger.type == "aura" and trigger.unit == "nameplate" then
    ...
    local maxAuras = math.floor(tonumber(frame.nameplateMaxAurasInput:GetText()) or trigger.nameplateMaxAuras or 3)
  ...
  if trigger.type == "aura_list" and UnitAuraList and UnitAuraList.ApplySourceValue then
    ...
    UnitAuraList:ApplyMaxDuration(trigger, frame.auraListMaxDurationInput:GetText())
    UnitAuraList:ApplyMaxRows(trigger, frame.auraListMaxRowsInput:GetText())
  end
  ...
  elseif trigger.type == "aura" then
    if trigger.unit == "nameplate" then
      ...
      trigger.itemId = nil
      trigger.itemName = nil
      frame.resolvedLabel:SetText("|cff88ff88Native hostile NPC buff display.|r Filters selected below are combined with AND.")
    elseif input == "" then
      trigger.spellIDs = nil
      trigger.spellNames = nil
      trigger.spellId = nil
...
function Panel:Create(parent)
  ...
  frame.auraListSortModeLabel = Frames.CreateLabel(frame, "Sort Order", "GameFontNormal")
  frame.auraListSortModeLabel:SetPoint("TOPLEFT", frame.auraListSourceLabel, "TOPLEFT", 230, 0)
  frame.auraListSortModeDropDown = Frames.CreateDropdown(frame, 210)
  frame.auraListSortModeDropDown:SetPoint("TOPLEFT", frame.auraListSortModeLabel, "BOTTOMLEFT", -14, -4)
  frame.auraListMaxDurationLabel = Frames.CreateLabel(
    frame, "Maximum Original Duration (seconds)", "GameFontNormal")
  frame.auraListMaxDurationLabel:SetPoint("TOPLEFT", frame.auraListSourceDropDown, "BOTTOMLEFT", 14, -12)
  frame.auraListMaxDurationInput = Frames.CreateInput(frame, 120, 24)
  ...
  frame.auraListMaxRowsLabel = Frames.CreateLabel(
    frame, "Maximum Displayed Rows (0 = unlimited)", "GameFontNormal")
  frame.auraListMaxRowsLabel:SetPoint("TOPLEFT", frame.auraListSortModeDropDown, "BOTTOMLEFT", 14, -12)
  frame.auraListMaxRowsInput = Frames.CreateInput(frame, 120, 24)
  frame.auraListMaxRowsInput:SetPoint("TOPLEFT", frame.auraListMaxRowsLabel, "BOTTOMLEFT", 0, -6)
  frame.auraListFilterHint = Frames.CreateLabel(
    frame,
    "Blizzard performs the filtering, sorting, and row limit. 0 means unlimited; a nonzero duration cap also excludes permanent auras.",
    "GameFontDisableSmall"
  )
  frame.auraListFilterHint:SetPoint("TOPLEFT", frame.auraListMaxDurationInput, "BOTTOMLEFT", 0, -8)
  frame.auraListFilterHint:SetWidth(470)
  ...
  frame.auraListHideBlizzardBuffsCheck:SetPoint("TOPLEFT", frame.auraListFilterHint, "BOTTOMLEFT", 0, -8)
  frame.auraListHideBlizzardDebuffsCheck = Frames.CreateCheckbox(frame, "Hide Blizzard Debuffs")
  frame.auraListHideBlizzardDebuffsCheck:SetPoint("TOPLEFT", frame.auraListFilterHint, "BOTTOMLEFT", 0, -8)
  ...
  frame.nameplateFilterBox:SetBackdrop({
    bgFile = "Interface\\Buttons\\WHITE8x8",
    edgeFile = "Interface\\Buttons\\WHITE8x8",
    edgeSize = 1,
  })
  frame.nameplateFilterBox:SetBackdropColor(0.08, 0.10, 0.15, 0.96)
  frame.nameplateFilterBox:SetBackdropBorderColor(0.22, 0.28, 0.36, 1)
  ...
  frame.nameplateFilterIntro = Frames.CreateLabel(
    ...
    "These choices are Blizzard aura classifications, not separate display modes.",
  ...
  frame.nameplateHint = Frames.CreateLabel(
    frame.nameplateFilterBox,
    "Checked categories are combined with AND. For Stealable OR Boss / encounter, use two displays. With nothing selected, no icons are shown.",
    "GameFontDisableSmall"
  )
...
function Panel:Refresh(aura)
  ...
  local triggers = EnsureAuraTriggers(aura)
  local trigger, triggerIndex = self:GetSelectedTrigger(aura)
  local isNameplateAura = trigger.type == "aura" and (trigger.unit or "player") == "nameplate"
  UpdatePrimaryTriggerLayout(self.frame, aura)
  SetDropdownValue(self.frame.triggerSelectDropDown, triggerIndex, function()
    return GetTriggerDropdownValues(aura)
  end)
  self.frame.triggerSelectLabel:SetShown(not isNameplateAura)
  self.frame.triggerSelectDropDown:SetShown(not isNameplateAura)
  self.frame.addTriggerButton:SetShown(not isNameplateAura)
  self.frame.removeTriggerButton:SetShown(#triggers > 1 and not isNameplateAura)
  self.frame.typeLabel:SetShown(not isNameplateAura)
  self.frame.typeDropDown:SetShown(not isNameplateAura)
  self.frame.opLabel:SetShown(#triggers > 1 and not isNameplateAura)
  self.frame.opDropDown:SetShown(#triggers > 1 and not isNameplateAura)
  ...
  if trigger.type == "aura" then
    if isNameplateAura then
      ...
      self.frame.resolvedLabel:SetText("|cff88ff88Native hostile NPC buff display.|r Filters selected below are combined with AND.")
    else
  ...
  self.frame.auraCastByMeCheck:SetShown(isAura and not isNameplateAura and (trigger.unit or "player") ~= "player")
  ...
  self.frame.argLabel:SetShown(not isDeathAlert and not isAuraList and not isTrinketCooldown and not isNameplateAura)
  self.frame.argInput:SetShown(not isDeathAlert and not isAuraList and not isTrinketCooldown and not isNameplateAura)
  self.frame.resolvedLabel:SetShown(not isNameplateAura)
  ...
  self.frame.nameplateMaxAurasInput:SetText(isNameplateAura and tostring(trigger.nameplateMaxAuras or 3) or "")
  ...
  elseif isAuraList then
    self.frame.argInput:SetText("")
    self.frame.resolvedLabel:SetText("|cff88ff88Blizzard natively builds and updates this unit's buff or debuff list.|r")
  end
